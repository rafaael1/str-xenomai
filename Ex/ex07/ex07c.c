...
int read_SWITCH() {
	...
	*/
	ret_SWITCH = read(fd_SWITCH, &value_SWITCH, sizeof(value_SWITCH));
	return value_SWITCH;
}
...
void verify_interrupt_switch(void *arg) {
	int num=*(int *)arg;
	printf("Task  : %d\n",num);
	//rt_sem_p(&mysync,0);
	...
	while(1) {
		...
		if (read_SWITCH() == 1){
			count++;
			write_LED((1<<value_LED)&1);
			printf("SWITCH_INTERRUPT_COUNT: %d\n",count);
		}		
		//rt_task_wait_period(NULL);
	}
	printf("End Task  : %d\n",num);
...
void startup() {
	...
	rt_task_start(&demo_task[i], &verify_interrupt_switch, &i);
